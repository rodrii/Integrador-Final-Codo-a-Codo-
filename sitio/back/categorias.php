...
if (isset($_REQUEST['mode'])=='remove') {
	    	...
	    	$strsql = "Delete from categorias where id=".addslashes($_REQUEST['id']);
...
for ($i=$reg1; $i<min($reg1+$tampag, $total); $i++) {
	...
	?>
                                <tr>
								    ...
								    <td class="center"> <a class="btn btn-info" href="categorias-editar.php?id=<?php echo $row['id']; ?>&action=modificar"> <i class="icon-edit icon-white"></i> Editar </a>  <a class="btn btn-danger" href="javascript: deleteAlert('<?php echo $row['categoria']; ?>','<?php echo $row['id'];?>')"> <i class="icon-trash icon-white"></i> Borrar </a> </td>
								</tr>
                                <?php }  ?>
							  </tbody>
						 </table>  
